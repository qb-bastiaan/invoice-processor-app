 ...
 */
export async function prepareInvoiceDataForGemini(
  filePath: string
...
export async function callGeminiApi(
  ...
  try {
    ...
    const timeoutMs = process.env.GEMINI_API_TIMEOUT_MS
      ? parseInt(process.env.GEMINI_API_TIMEOUT_MS, 10)
      : 30000;
    ...
    try {
      result = await Promise.race([
        ...
        new Promise((_, reject) =>
          setTimeout(
            () =>
              reject(
                new Error(`Gemini API call timed out after ${timeoutMs}ms`)
              ),
            timeoutMs
          )
        ),
      ...
      console.error('Gemini API call timed out or failed:', err);
      throw err;
    }
    ...
    if (
      ...
      console.error(
        'Gemini API response did not contain the expected text part:',
        candidate
      );
      return null;
    }
