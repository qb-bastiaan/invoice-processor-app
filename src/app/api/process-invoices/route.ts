...
import { getGeminiSystemPrompt, prepareInvoiceDataForGemini, callGeminiApi } from '@/lib/gemini-utils';
...
const SCHEMA_PATH = path.join(process.cwd(), 'config', 'invoice_output_schema.json');
...
export async function GET(request: Request): Promise<Response> {
  ...
  const stream = new ReadableStream({
    async start(controller) {
      ...
      const sendEvent = (data: {type: string, [key: string]: any}) => {
        ...
        try {
          ...
          if (e.name === 'TypeError' && e.message.includes('Invalid state')) {
            isStreamClosed = true;
          } else {
            console.error(`Unhandled error enqueuing data for SSE (event type: ${data.type}):`, e);
          }
        }
      };
      ...
      try {
        ...
        if (supportedFiles.length === 0) {
          sendEvent({ type: "error", message: 'No supported files (PDF, JPEG, JPG) found in input-files directory.' });
          closeStream(); 
          return; 
        }
        ...
        if (startIndex === 0) {
            sendEvent({ type: "info", message: `Found ${supportedFiles.length} supported files. Starting batch...`, totalFiles: supportedFiles.length });
        } else {
            sendEvent({ type: "info", message: `Requesting processing for file at index ${startIndex}. Total files: ${supportedFiles.length}`});
        }
        ...
        currentProcessingResult = { // Assign to currentProcessingResult in this scope
        ...
        sendEvent({ type: "file_update", progress: { current: i + 1, total: supportedFiles.length }, data: { ...currentProcessingResult } });

        try { // Inner try-catch for this specific file's processing
          const filePath = path.join(INPUT_FILES_DIR, fileName); // fileName is now in scope
          const { mimeType, data: base64Data } = await prepareInvoiceDataForGemini(filePath);
          ...
          sendEvent({ type: "file_update", progress: { current: i + 1, total: supportedFiles.length }, data: { ...currentProcessingResult } });
          const pass1ResponseText = await callGeminiApi(systemPrompt, mimeType, base64Data, [pass1Prompt]);

          if (!pass1ResponseText) throw new Error(`Gemini (Pass 1: Structural Analysis) returned no text.`);
          ...
          sendEvent({ type: "file_update", progress: { current: i + 1, total: supportedFiles.length }, data: { ...currentProcessingResult } });
          const pass2ResponseText = await callGeminiApi(systemPrompt, mimeType, base64Data, [`Context from Pass 1: ${pass1ResponseText}`, pass2Prompt]);
          
          if (!pass2ResponseText) throw new Error(`Gemini (Pass 2: Region Identification) returned no text.`);
          ...
          currentProcessingResult.status = "gemini_pass3_json_extraction_calling";
          sendEvent({ type: "file_update", progress: { current: i + 1, total: supportedFiles.length }, data: { ...currentProcessingResult } });
          const pass3GeminiResponseText = await callGeminiApi(systemPrompt, mimeType, base64Data, [`Context from Pass 1: ${pass1ResponseText}`, `Context from Pass 2: ${pass2ResponseText}`, pass3Prompt]);

          if (!pass3GeminiResponseText) throw new Error(`Gemini (Pass 3: JSON Extraction) returned no text.`);
          currentProcessingResult.geminiResponsePreview = pass3GeminiResponseText.substring(0, 100) + "...";
          currentProcessingResult.status = "gemini_pass3_json_extraction_complete";
          sendEvent({ type: "file_update", progress: { current: i + 1, total: supportedFiles.length }, data: { ...currentProcessingResult } });
          
          let parsedDataAttempt: any;
          try {
            const cleanedResponseText = pass3GeminiResponseText.replace(/^```json\s*|```\s*$/g, '').trim();
          ...
          currentProcessingResult.parsedDataSnippet = JSON.stringify(parsedDataAttempt, null, 2).substring(0,250) + "\n...";
          ...
          if (isValid) {
            ...
            validationDetails = { status: "failed", errors_summary: errorSummary, errors_list: validateFunction.errors };
          ...
          const supplierName = parsedDataAttempt.supplier_name?.replace(/[^a-z0-9]/gi, '_') || "UnknownSupplier";
          const invoiceDateStr = parsedDataAttempt.invoice_date || new Date().toISOString().split('T')[0];
          const invoiceNumber = parsedDataAttempt.invoice_number?.replace(/[^a-z0-9]/gi, '_') || "NoInvoiceNumber";
          ...
          try {
            await fs.writeFile(outputFilePath, JSON.stringify(parsedDataAttempt, null, 2));
            currentProcessingResult.status = "saved_successfully";
          } catch (saveError) {
            throw new Error(`File save error: ${saveError instanceof Error ? saveError.message : String(saveError)}`);
          }

        } catch (fileProcessingError: any) { 
          console.error(`Error processing file ${fileName}:`, fileProcessingError.message); // fileName is in scope
          if(currentProcessingResult) { // Check if currentProcessingResult is not null
            currentProcessingResult.errorDetail = fileProcessingError.message;
            currentProcessingResult.status = currentProcessingResult.status.startsWith("error_") ? currentProcessingResult.status : "error_processing_file";
            currentProcessingResult.parsedData = { error: "Processing failed for this file", details: fileProcessingError.message };
        ...
        sendEvent({ 
            ...
            totalFiles: supportedFiles.length,
            processedFileResult: currentProcessingResult ? { ...currentProcessingResult } : null // Add the result here
        });

      } catch (error) { 
        console.error('Error in /api/process-invoices SSE stream logic (outer try):', error);
