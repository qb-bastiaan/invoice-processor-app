...
import LineItemsDisplay, {
  LineItem,
} from '@/components/custom/LineItemsDisplay'; // Re-enable import
...
export default function Home() {
  ...
  const [message, setMessage] = useState(
    'Page loaded. Step 3: Full onmessage state logic re-enabled.'
  );
  ...
  const [currentFilePreview, setCurrentFilePreview] = useState<{
    fileName: string;
    mimeType: string;
    data: string;
  } | null>(null);
  const [currentFileExtractedData, setCurrentFileExtractedData] =
    useState<ApiProcessingResult | null>(null);
  ...
  const startProcessingFileAtIndex = (index: number) => {
    ...
    if (totalFilesToProcess > 0) {
      ...
      // For the very first file, show some initial progress
      ...
      try {
        ...
        switch (eventData.type) {
          case 'info':
            ...
            if (index === 0 && eventData.totalFiles !== undefined) {
              // Check if it's the first file call (index 0)
          ...
          case 'file_update':
            ...
            if (totalFilesToProcess > 0) {
              // Ensure totalFilesToProcess is set before calculating progress
              setCurrentProgress({
                current: index + 1,
          ...
          default:
            console.log(
              'Received unhandled SSE event type:',
              eventData.type,
              eventData
            );
        }
  ...
  const handleStartBatchProcessing = () => {
    ...
    console.log(
      'handleStartBatchProcessing: Calling startProcessingFileAtIndex(0)'
    );
  ...
  const handleAcceptAndNext = () => {
    console.log(
      'handleAcceptAndNext called. Current nextFileIndexToProcess:',
      nextFileIndexToProcess,
      'Total files:',
      totalFilesToProcess
    );
    if (isWaitingForAcceptance) {
      ...
      if (nextIndexToActuallyProcess < totalFilesToProcess) {
        setMessage(
          `Proceeding to file ${nextIndexToActuallyProcess + 1} of ${totalFilesToProcess}`
        );
        startProcessingFileAtIndex(nextIndexToActuallyProcess);
      } else {
        setMessage(
          'All files in the batch have been processed. (from handleAcceptAndNext)'
        );
        setIsLoading(false);
        setIsBatchProcessingActive(false);
        setIsWaitingForAcceptance(false);
      ...
      console.warn(
        'handleAcceptAndNext called when not waiting for acceptance.'
      );
    }
  };

  const handleLineItemsDataChange = (updatedLineItems: LineItem[]) => {
    setCurrentFileExtractedData((prevData) => {
      if (!prevData || !prevData.parsedData) {
        // This case should ideally not happen if LineItemsDisplay is only shown when there's parsedData
        console.warn(
          'handleLineItemsDataChange called but prevData or prevData.parsedData is null/undefined'
        );
        return prevData;
      }
      return {
    ...
    <div className="flex flex-col items-center min-h-screen bg-background text-foreground p-4 sm:p-8 font-[family-name:var(--font-sans)]">
      <header className="w-full max-w-5xl mb-8 sm:mb-12 text-center">
        ...
        <p className="text-md sm:text-lg text-gray-400">
          Utilizing Google Gemini for intelligent data extraction from PDF and
          JPEG invoices.
        </p>
      </header>

      <main className="w-full max-w-7xl mx-auto flex flex-col items-stretch gap-6 sm:gap-8">
        <section className="w-full p-4 sm:p-6 bg-card text-card-foreground rounded-2xl shadow-sm">
            ...
            !isWaitingForAcceptance && (
              <div className="w-full mt-4">
                <p className="text-sm text-gray-300 mb-1 text-center sm:text-left">
                  Processing File: {currentProgress.current} of{' '}
          ...
          {message && (
            <div
              className={`mt-4 p-3 rounded-md text-center w-full ${message.toLowerCase().includes('error') ? 'bg-red-900 text-red-100' : isLoading || isWaitingForAcceptance ? 'bg-blue-900 text-blue-100' : 'bg-gray-700 text-gray-200'} border border-gray-600`}
            >
        ...
        {(currentFilePreview || currentFileExtractedData) &&
          isBatchProcessingActive && (
            <>
              <section className="w-full p-4 sm:p-6 bg-card text-card-foreground rounded-2xl shadow-sm mt-6 flex flex-col md:flex-row gap-6">
                <div className="w-full md:w-1/2 p-4">
                  <h3 className="text-lg sm:text-xl font-semibold mb-3 sm:mb-4">
                    Invoice Preview:{' '}
                    <span className="font-normal text-muted-foreground">
                      {currentFilePreview?.fileName ||
                        currentFileExtractedData?.fileName ||
                        'N/A'}
                    </span>
                  </h3>
                  <InvoiceViewer
                ...
                <div className="w-full md:w-1/2 p-4">
                  ...
                  <LineItemsDisplay
                    extractedData={currentFileExtractedData}
                    onLineItemsChange={handleLineItemsDataChange}
                  />
                </div>
              </section>
            </>
        ...
        <section className="w-full mt-4 sm:mt-8 p-4 sm:p-6 bg-card text-card-foreground border border-border rounded-2xl shadow-sm">
          ...
          <ul className="space-y-3 max-h-80 sm:max-h-96 overflow-y-auto">
            ...
            {processingResults.length > 0 ? (
              processingResults.map((result, idx) => (
                <li
                  key={`${result.fileName || `file-${idx}`}-${idx}`}
                  className="p-3 bg-background rounded-md border border-border shadow-sm"
                >
                  ...
                  <p className="text-sm">
                    Status:{' '}
                    <span
                      className={`font-medium ${result.status && result.status.toLowerCase().includes('error') ? 'text-destructive' : 'text-primary'}`}
                    >
                      {result.status || 'N/A'}
                    </span>
                  </p>
              ...
              <li style={{ color: 'white', padding: '5px' }}>
